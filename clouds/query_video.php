			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="card">
						<div class="card-body">
							<div class="input-group mb-md">
                             <a href="video_control.php?tp=<?=$type_content;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descsearch = $_REQUEST['search'];
						...
						$sql = "SELECT * FROM video WHERE (title_video like '%$descsearch%') ORDER BY title_video DESC";
						}
						if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM video WHERE (title_video like '%$descsearch%') ORDER BY title_video DESC";
						}
					...
					if ($_REQUEST['Del'] == "del") {
						$code = $_REQUEST['code'];
						$sql = "DELETE FROM video WHERE id_video = '$code'";
						$result = mysqli_query($connectionSql, $sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
					    //if ($category_user_valid == 1) {
						//$sql = "SELECT * FROM video ORDER BY date_video DESC, id_video DESC";
						//}
						//if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM video, category_video WHERE video.id_category_video = category_video.id_category_video ORDER BY date_video DESC, id_video DESC";
						//}
				}
				...
				$category       	= $row["title_category_video"];
				$desc	        	= $row["desc_video"];
				$tags	        	= $row["tags_video"];
				$image	        	= $row["image_video"];
				$status             = $row["status_video"];
				...
				?>
                             <section class="card">
                                    ...
                                    <div class="col-md-9">
                                        <p class="pull-right">
                                         <a href="video_control.php?cod=<?=$code;?>" title="Editar"><span class="btn btn-warning"><i class="fas fa-edit"></i></span></a>
                                         <a href="query_video.php?Del=del&amp;code=<?=$code?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fas fa-trash-alt"></i></span></a>
                                        </p>
