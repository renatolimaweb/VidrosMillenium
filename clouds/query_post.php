			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="card">
						<div class="card-body">
							<div class="input-group mb-md">
                             <a href="post_control.php?tp=<?=$type_content;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descsearch = $_REQUEST['search'];
						...
						$sql = "SELECT * FROM post WHERE (title_post like '%$descsearch%') ORDER BY title_post DESC";
						}
						if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM post WHERE (title_post like '%$descsearch%') ORDER BY title_post DESC";
						}
					...
					if ($_REQUEST['Del'] == "del") {
						$code = $_REQUEST['code'];
						$sql = "DELETE FROM post WHERE id_post = '$code'";
						...
						//}
						//if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM post, category_post WHERE post.id_category_post = category_post.id_category_post ORDER BY date_post DESC, id_post DESC";
						//}
				}
				...
				$category       	= $row["title_category_post"];
				$desc	        	= $row["desc_post"];
				$tags	        	= $row["tags_post"];
				...
				?>
                             <section class="card">
                                    ...
                                    <div class="col-md-9">
                                        <p class="pull-right">
                                         <a href="post_control.php?cod=<?=$code;?>" title="Editar"><span class="btn btn-warning"><i class="fas fa-edit"></i></span></a>
                                         <a href="query_post.php?Del=del&amp;code=<?=$code?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fas fa-trash-alt"></i></span></a>
                                        </p>
