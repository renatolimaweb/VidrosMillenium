...
<html class="fixed dark">
	<head>
		...
		<title>Categoria de Vídeos - Sistema Gerenciador de Conteúdo</title>
			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="card">
						<div class="card-body">
							<div class="input-group mb-md">
                             <a href="category_video_control.php?tp=<?=$type_content;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descsearch = $_REQUEST['search'];
						...
						$sql = "SELECT * FROM category_video WHERE (title_category_video  like '%$descsearch%') ORDER BY title_category_video DESC";
						}
						if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM category_video WHERE (title_category_video  like '%$descsearch%') ORDER BY title_category_video DESC";
						}
					...
					if ($_REQUEST['Del'] == "del") {
						$code = $_REQUEST['code'];
						$sql = "DELETE FROM category_video WHERE id_category_video = '$code'";
						...
						//}
						//if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM category_video ORDER BY id_category_video DESC";
						//}
				}
				...
				$code 			    = $row["id_category_video"];
				$title	        	= $row["title_category_video"];
				$status             = $row["status_category_video"];
				...
				?>
                             <section class="card">
                                    ...
                                    <div class="col-md-12">
                                        <p class="pull-right">
                                         <a href="category_video_control.php?cod=<?=$code;?>" title="Editar"><span class="btn btn-warning"><i class="fas fa-edit"></i></span></a>
                                         <a href="query_category_video.php?Del=del&amp;code=<?=$code?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fas fa-trash-alt"></i></span></a>
                                        </p>
                                        <p><i class="fa fa-datebase"></i> #<?=$code;?></p>
										<h4><i class="fa fa-tag"></i> <?=$title;?></h4>
                                        <p>
                                        <? if($status == 1){ ?>
