			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="card">
						<div class="card-body">
							<div class="input-group mb-md">
                             <a href="link_control.php?tp=<?=$type_content;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descsearch = $_REQUEST['search'];
						...
						$sql = "SELECT * FROM link WHERE (title_link like '%$descsearch%') ORDER BY title_link DESC";
						}
						if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM link WHERE (title_link like '%$descsearch%') ORDER BY title_link DESC";
						}
					...
					if ($_REQUEST['Del'] == "del") {
						$code = $_REQUEST['code'];
						$sql = "DELETE FROM link WHERE id_link = '$code'";
						...
						//}
						//if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM link ORDER BY id_link DESC";
						//}
				}
				...
				$code 			    = $row["id_link"];
				$title	        	= $row["title_link"];
				$status             = $row["status_link"];
				...
				?>
                             <section class="card">
                                    ...
                                    <div class="col-md-12">
                                        <p class="pull-right">
                                         <a href="link_control.php?cod=<?=$code;?>" title="Editar"><span class="btn btn-warning"><i class="fas fa-edit"></i></span></a>
                                         <a href="query_link.php?Del=del&amp;code=<?=$code?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fas fa-trash-alt"></i></span></a>
                                        </p>
