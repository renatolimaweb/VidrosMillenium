			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="card">
						<div class="card-body">
							<div class="input-group mb-md">
                             <a href="slideshow_control.php?tp=<?=$type_content;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descsearch = $_REQUEST['search'];
						...
						$sql = "SELECT * FROM slideshow WHERE (title_slideshow like '%$descsearch%') ORDER BY title_slideshow DESC";
						}
						if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM slideshow WHERE (title_slideshow like '%$descsearch%') ORDER BY title_slideshow DESC";
						}
					...
					if ($_REQUEST['Del'] == "del") {
						$code = $_REQUEST['code'];
						$sql = "DELETE FROM slideshow WHERE id_slideshow = '$code'";
						...
						//}
						//if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM slideshow ORDER BY id_slideshow DESC";
						//}
				}
				...
				$code 			    = $row["id_slideshow"];
				$title	        	= $row["title_slideshow"];
				$desc	        	= $row["desc_slideshow"];;
				$image	        	= $row["image_slideshow"];
				$status             = $row["status_slideshow"];
				...
				?>
                             <section class="card">
                                    ...
                                    <div class="col-md-9">
                                        <p class="pull-right">
                                         <a href="slideshow_control.php?cod=<?=$code;?>" title="Editar"><span class="btn btn-warning"><i class="fas fa-edit"></i></span></a>
                                         <a href="query_slideshow.php?Del=del&amp;code=<?=$code?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fas fa-trash-alt"></i></span></a>
                                        </p>
