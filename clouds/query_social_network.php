			...
			<div class="inner-wrapper">
				...
				<section role="main" class="content-body">
                    ...
                    <section class="card">
						<div class="card-body">
							<div class="input-group mb-md">
                             <a href="social_network_control.php?tp=<?=$type_content;?>"><span class="btn btn-lg btn-success"><i class="fa fa-check"></i> Cadastrar</span></a>
							</div>
						</div>
					</section>
				...
				if ($_REQUEST["Consulta"] != "") {
					$descsearch = $_REQUEST['search'];
						...
						$sql = "SELECT * FROM social_network WHERE (title_social_network like '%$descsearch%') ORDER BY title_social_network DESC";
						}
						if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM social_network WHERE (title_social_network like '%$descsearch%') ORDER BY title_social_network DESC";
						}
					...
					if ($_REQUEST['Del'] == "del") {
						$code = $_REQUEST['code'];
						$sql = "DELETE FROM social_network WHERE id_social_network = '$code'";
						$result = mysqli_query($connectionSql, $sql) or die ("N&atilde;o foi poss&iacute;vel realizar a consulta ao banco de dados");
					}
					    //if ($category_user_valid == 1) {
						//$sql = "SELECT * FROM page ORDER BY date_page DESC, id_page DESC";
						//}
						//if ($category_user_valid <> 1) {
						$sql = "SELECT * FROM social_network ORDER BY id_social_network DESC";
						//}
				}
				...
				$code 			    = $row["id_social_network"];
				$title	        	= $row["title_social_network"];
				$status             = $row["status_social_network"];
				...
				?>
                             <section class="card">
                                    ...
                                    <div class="col-md-12">
                                        <p class="pull-right">
                                         <a href="social_network_control.php?cod=<?=$code;?>" title="Editar"><span class="btn btn-warning"><i class="fas fa-edit"></i></span></a>
                                         <a href="query_social_network.php?Del=del&amp;code=<?=$code?>" onclick="return confirm('Deseja excluir ?')" title="Excluir"><span class="btn btn-danger"><i class="fas fa-trash-alt"></i></span></a>
                                        </p>
